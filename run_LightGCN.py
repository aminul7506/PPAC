...
def train_model(dataset, device, i_num, test_records, train_records, model, params, graph, global_pop):
    ...
    for epoch in range(total_epoch):
        ...
        recall = sum(results['recall'][TOP_Ks[0]]) / len(results['recall'][TOP_Ks[0]])
        if recall > best_res:
            best_res = recall
            best_epoch = epoch
        ...
        print('Epoch [{}/{}], Loss: {:.4f}, Recall@{}: {:.4f}, Time: {:.2f}s'.format(epoch + 1, total_epoch,
                                                                                   total_loss / runs,
                                                                                   TOP_Ks[0],
                                                                                   recall, time.time() - tim1))
    print(f'Best Recall@{TOP_Ks[1]}: {best_res:.4f}, Best epoch: {best_epoch}')
...
def using_new_model(sys_paras):
    ...
    if sys_paras.test_pru:
        ...
        for i, topk in enumerate(TOP_Ks):
            print(f'Precision@{topk}: {sum(res["precision"][topk]) / len(res["precision"][topk]):.6f}  '
                  f'Recall@{topk}: {sum(res["recall"][topk]) / len(res["recall"][topk]):.6f}  '
                  f'NDCG@{topk}: {sum(res["ndcg"][topk]) / len(res["ndcg"][topk]):.6f} '
                  f'HR@{topk}: {sum(res["hr"][topk]) / len(res["hr"][topk]):.6f} '
                  f'ARP@{topk}: {sum(res["arp"][topk]) / len(res["arp"][topk]):.6f};  ')
