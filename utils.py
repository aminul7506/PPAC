...
def RecallPrecisionHR_ATk(test_data, r, k):
    """
    ...
    hit_ratio = np.sum(right_pred > 0)
    return {'recall': recall, 'precision': precis, 'hr': hit_ratio}


def RecallPrecisionHR_ATk_cointer(test_data, r, k):
    """
    test_data should be a list? cause users may have different amount of pos items. shape (test_batch, k)
    pred_data : shape (test_batch, k) NOTE: pred_data should be pre-sorted
    k : top-k
...
def test_one_batch(X, topks, global_pop=None):
    ...
    pre, recall, ndcg, arp, hr = [], [], [], [], []
    for k in topks:
        ret = RecallPrecisionHR_ATk_cointer(groundTrue, r, k)
    ...
    with torch.no_grad():
        ...
        for result in pre_results:
            for idx, topk in enumerate(TOP_Ks):
                ...
                results['hr'][topk].extend(result['hr'][idx])
